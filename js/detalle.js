			...
			.then((data) => {
				...
				if (productos.length === 0) {
					...
					if (producto_seleccionado) {
						document.getElementById('titulo-card').innerText =
							producto_seleccionado.nombre;
						document.getElementById('descripcion-card').innerText =
							producto_seleccionado.descripcion;
						document.getElementById('categoria-card').innerText =
							displayCategoria.charAt(0).toUpperCase() +
							displayCategoria.slice(1);
						document.getElementById('precio-card').innerText =
							producto_seleccionado.precio;
						document.getElementById('imagen1').src =
							producto_seleccionado.imagen1;
						document.getElementById('imagen2').src =
							producto_seleccionado.imagen2; // Añade imágenes adicionales si es necesario
						document.getElementById('imagen3').src =
							producto_seleccionado.imagen3; // Añade imágenes adicionales si es necesario
					}
				}
			})
			.catch((error) => {
